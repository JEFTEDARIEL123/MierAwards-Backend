...
app.use((req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', 'https://tu-frontend.com'); // Reemplaza con la URL de tu frontend
  ...
  Form.findOne({ nickname })
    .then((existingForm) => {
      if (existingForm) {
        ...
        if (abuelo) {
          existingForm.abuelo = abuelo;
        }

        if (artista) {
          existingForm.artista = artista;
        }

        if (hater) {
          existingForm.hater = hater;
        }

        if (duo) {
          existingForm.duo = duo;
        }
